import React from 'react';
import SourceCard from './SourceCard'

export default function SingleQuestionCard() {
  return (
    <>
          <section className='brain_header'>
              <h2>what is data poisoning in ai</h2>
          </section>
          <section className='brain_source'>
              <div className='brain_title'>
                  <span>
                      <svg fill="#ffffff" height="20px" width="20px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                          viewBox="0 0 512 512" xml:space="preserve">
                          <g>
                              <g>
                                  <path d="M185.86,0H42.837C23.18,0,5.953,15.183,5.953,34.846v143.023c0,19.663,17.227,36.456,36.884,36.456H185.86
			c19.657,0,34.419-16.794,34.419-36.456V34.846C220.279,15.183,205.517,0,185.86,0z M196.465,177.869
			c0,6.523-4.076,12.643-10.605,12.643H42.837c-6.529,0-13.07-6.119-13.07-12.643V34.846c0-6.524,6.541-11.032,13.07-11.032H185.86
			c6.529,0,10.605,4.509,10.605,11.032V177.869z"/>
                              </g>
                          </g>
                          <g>
                              <g>
                                  <path d="M185.86,297.674H42.837c-19.657,0-36.884,15.183-36.884,34.846v143.023C5.953,495.206,23.18,512,42.837,512H185.86
			c19.657,0,34.419-16.794,34.419-36.456V332.521C220.279,312.858,205.517,297.674,185.86,297.674z M196.465,475.544
			c0,6.523-4.076,12.643-10.605,12.643H42.837c-6.529,0-13.07-6.119-13.07-12.643V332.521c0-6.524,6.541-11.032,13.07-11.032H185.86
			c6.529,0,10.605,4.509,10.605,11.032V475.544z"/>
                              </g>
                          </g>
                          <g>
                              <g>
                                  <path d="M471.628,0H328.605c-19.657,0-36.884,15.183-36.884,34.846v143.023c0,19.663,17.227,36.456,36.884,36.456h143.023
			c19.657,0,34.419-16.794,34.419-36.456V34.846C506.047,15.183,491.285,0,471.628,0z M482.233,177.869
			c0,6.523-4.075,12.643-10.605,12.643H328.605c-6.529,0-13.07-6.119-13.07-12.643V34.846c0-6.524,6.541-11.032,13.07-11.032
			h143.023c6.529,0,10.605,4.509,10.605,11.032V177.869z"/>
                              </g>
                          </g>
                          <g>
                              <g>
                                  <path d="M471.628,297.674H328.605c-19.657,0-36.884,15.183-36.884,34.846v143.023c0,19.663,17.227,36.456,36.884,36.456h143.023
			c19.657,0,34.419-16.794,34.419-36.456V332.521C506.047,312.858,491.285,297.674,471.628,297.674z M482.233,475.544
			c0,6.523-4.075,12.643-10.605,12.643H328.605c-6.529,0-13.07-6.119-13.07-12.643V332.521c0-6.524,6.541-11.032,13.07-11.032
			h143.023c6.529,0,10.605,4.509,10.605,11.032V475.544z"/>
                              </g>
                          </g>
                      </svg>
                  </span>
                  <label>Source</label>
              </div>
              <div className='brain_source__list'>
                  <SourceCard />
                  <SourceCard />

                  <SourceCard />

              </div>
          </section>
          <section className='brain_content'>
              <div className='brain_title'>
                  <span>
                      <svg fill="#ffffff" width="20px" height="20px" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
                          <title>book-open</title>
                          <path d="M30.527 5.467c-0.147-0.118-0.33-0.196-0.531-0.216l-0.004-0-3.242 0.033v-3.283c-0-0.208-0.085-0.395-0.221-0.531v0c-0.134-0.135-0.319-0.219-0.524-0.219-0.003 0-0.007 0-0.010 0h0c-0.296-0.024-0.64-0.038-0.988-0.038-3.501 0-6.678 1.386-9.012 3.64l0.004-0.004c-2.33-2.25-5.507-3.637-9.008-3.637-0.348 0-0.693 0.014-1.034 0.041l0.045-0.003c-0.206 0.006-0.393 0.088-0.533 0.219l0-0c-0.136 0.136-0.221 0.323-0.221 0.531 0 0 0 0 0 0v-0 3.283l-3.242-0.033c-0.003-0-0.007-0-0.011-0-0.205 0-0.39 0.083-0.524 0.217v0c-0.137 0.136-0.223 0.324-0.223 0.533 0 0 0 0 0 0v0 24c0 0.414 0.336 0.749 0.75 0.749 0.13 0 0.252-0.033 0.359-0.091l-0.004 0.002c1.904-1.129 4.196-1.797 6.645-1.797 2.443 0 4.731 0.665 6.693 1.823l-0.061-0.034c0.058 0.034 0.125 0.061 0.196 0.077l0.005 0.001c0.028 0.007 0.060 0.013 0.094 0.016l0.002 0h0.004l0.069 0.004 0.013-0.004c0.13-0.001 0.253-0.036 0.359-0.096l-0.004 0.002c1.901-1.124 4.189-1.788 6.632-1.788 2.449 0 4.742 0.667 6.707 1.83l-0.061-0.034c0.102 0.058 0.224 0.092 0.354 0.092 0.142 0 0.275-0.041 0.387-0.111l-0.003 0.002c0.22-0.133 0.365-0.371 0.365-0.642 0-0 0-0 0-0v0-24c0-0 0-0 0-0 0-0.208-0.085-0.397-0.223-0.533l-0-0zM6.75 2.766c0.166-0.009 0.36-0.014 0.556-0.014 3.142 0 5.969 1.346 7.937 3.494l0.007 0.008v21.837c-2.233-1.764-5.090-2.83-8.195-2.83-0.107 0-0.214 0.001-0.321 0.004l0.016-0zM2.75 28.793v-22.035l2.5 0.025v19.217c0 0.413 0.335 0.749 0.748 0.75h0c0.088-0.002 0.192-0.003 0.296-0.003 1.834 0 3.596 0.308 5.238 0.876l-0.112-0.034c-0.729-0.13-1.569-0.204-2.426-0.204-2.269 0-4.416 0.519-6.33 1.445l0.087-0.038zM16.75 6.253c1.975-2.155 4.803-3.502 7.945-3.502 0.195 0 0.39 0.005 0.582 0.015l-0.027-0.001v22.498c-0.091-0.002-0.198-0.003-0.305-0.003-3.105 0-5.962 1.066-8.223 2.851l0.028-0.021zM29.25 28.793c-1.831-0.877-3.978-1.397-6.244-1.414l-0.006-0c-0.855 0-1.691 0.077-2.503 0.224l0.085-0.013c1.529-0.534 3.291-0.843 5.126-0.843 0.103 0 0.207 0.001 0.309 0.003l-0.015-0c0.413-0.001 0.748-0.336 0.748-0.75 0-0 0-0 0-0v0-19.217l2.5-0.025z"></path>
                      </svg>
                  </span>
                  <label>Answer</label>
              </div>
              <div className='brain_content__details'>
                  Data poisoning in AI refers to a cybersecurity threat where training data for artificial intelligence (AI) models is intentionally tampered with by injecting false, biased, or malicious information. This manipulation of data can lead to incorrect outputs and biased decision-making processes, potentially causing real-life harm
                  3
                  2
                  . Data poisoning attacks can take various forms, such as injecting incorrect, biased, or misleading data into training sets, compromising the integrity and reliability of AI systems
                  4
                  . This type of attack can have serious implications, affecting the effectiveness and trustworthiness of AI models, especially when even a small percentage of the data is poisoned
                  4
                  .
              </div>
          </section>
          <section className='brain_links'>
              <div className='row'>
                  <div className='col-6 brain_links__btns'>
                      <div className='brain_links__btn'>
                          <span className='brain_links__btn_icon'>
                              <svg width="20px" height="20px" viewBox="0 -0.5 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <path fill-rule="evenodd" clip-rule="evenodd" d="M14.734 15.8974L19.22 12.1374C19.3971 11.9927 19.4998 11.7761 19.4998 11.5474C19.4998 11.3187 19.3971 11.1022 19.22 10.9574L14.734 7.19743C14.4947 6.9929 14.1598 6.94275 13.8711 7.06826C13.5824 7.19377 13.3906 7.47295 13.377 7.78743V9.27043C7.079 8.17943 5.5 13.8154 5.5 16.9974C6.961 14.5734 10.747 10.1794 13.377 13.8154V15.3024C13.3888 15.6178 13.5799 15.8987 13.8689 16.0254C14.158 16.1521 14.494 16.1024 14.734 15.8974Z" stroke="#eff7ff9c" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                              </svg>
                          </span>
                          <span>
                              Share
                          </span>
                      </div>
                      <div className='brain_links__btn'>
                          <span className='brain_links__btn_icon'>
                              <svg fill="#eff7ff9c" height="16px" width="16px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                  viewBox="0 0 512 512" xml:space="preserve">
                                  <g>
                                      <g>
                                          <path d="M483.555,256c0-92.796-79.528-171.416-189.102-186.935c-5.194-0.732-9.981,2.87-10.722,8.055
			c-0.732,5.185,2.87,9.991,8.055,10.722C391.916,102.029,464.592,172.741,464.592,256c0,81.548-71.569,151.895-168.266,167.451
			l46.97-46.97c3.704-3.704,3.704-9.704,0-13.407c-3.704-3.704-9.704-3.704-13.408,0l-66.37,66.37
			c-3.704,3.704-3.704,9.704,0,13.407l66.37,66.37c1.852,1.852,4.278,2.778,6.704,2.778c2.426,0,4.852-0.926,6.704-2.778
			c3.704-3.704,3.704-9.704,0-13.407l-52.442-52.442C400.969,429.271,483.555,349.306,483.555,256z"/>
                                      </g>
                                  </g>
                                  <g>
                                      <g>
                                          <path d="M228.083,425.148c-103-11.287-180.675-84-180.675-169.148c0-77.356,62.541-143.685,151.592-164.192l-40.657,54.211
			c-3.148,4.185-2.296,10.13,1.898,13.268c1.704,1.278,3.694,1.898,5.676,1.898c2.889,0,5.731-1.306,7.592-3.796l56.889-75.852
			c2.667-3.556,2.509-8.481-0.389-11.852L173.12,3.316c-3.398-3.991-9.389-4.435-13.361-1.028
			c-3.982,3.407-4.444,9.389-1.028,13.361l47.403,55.312C102.454,90.151,28.445,166.341,28.445,256
			c0,94.842,84.935,175.657,197.564,188c0.352,0.037,0.704,0.055,1.046,0.055c4.778,0,8.88-3.592,9.417-8.444
			C237.037,430.398,233.287,425.722,228.083,425.148z"/>
                                      </g>
                                  </g>
                              </svg>
                          </span>
                          <span>Rewrite</span>
                      </div>
                  </div>
                  <div className='col-6 brain_links__btns brain_links__right_btns'>
                      <div className='brain_links__btn'>
                          <span className='brain_links__btn_icon'>
                              <svg width="16px" height="16px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <path fill-rule="evenodd" clip-rule="evenodd" d="M21 8C21 6.34315 19.6569 5 18 5H10C8.34315 5 7 6.34315 7 8V20C7 21.6569 8.34315 23 10 23H18C19.6569 23 21 21.6569 21 20V8ZM19 8C19 7.44772 18.5523 7 18 7H10C9.44772 7 9 7.44772 9 8V20C9 20.5523 9.44772 21 10 21H18C18.5523 21 19 20.5523 19 20V8Z" fill="#eff7ff9c" />
                                  <path d="M6 3H16C16.5523 3 17 2.55228 17 2C17 1.44772 16.5523 1 16 1H6C4.34315 1 3 2.34315 3 4V18C3 18.5523 3.44772 19 4 19C4.55228 19 5 18.5523 5 18V4C5 3.44772 5.44772 3 6 3Z" fill="#eff7ff9c" />
                              </svg>
                          </span>
                      </div>
                      <div className='brain_links__btn'>
                          <span className='brain_links__btn_icon'>
                              <svg width="16px" height="16px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                  <title />
                                  <g id="Complete">
                                      <g id="edit">
                                          <g>
                                              <path d="M20,16v4a2,2,0,0,1-2,2H4a2,2,0,0,1-2-2V6A2,2,0,0,1,4,4H8" fill="none" stroke="#eff7ff9c" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
                                              <polygon fill="none" points="12.5 15.8 22 6.2 17.8 2 8.3 11.5 8 16 12.5 15.8" stroke="#eff7ff9c" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
                                          </g>
                                      </g>
                                  </g>
                              </svg>
                          </span>
                      </div>
                  </div>
              </div>
          </section>
    </>
  )
}
